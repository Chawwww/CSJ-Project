...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const topicInput = document.getElementById('topic');
  if (topicInput) {
    // Auto-focus on the topic input when page loads
    topicInput.focus();
  ...
  // Initialize example topics
  initializeExampleTopics();
});

// Function to add example topic suggestions
function initializeExampleTopics() {
  const exampleTopics = [
    'Coral reefs', 
    'Amazon rainforest', 
    'Pollinators', 
  ...
  exampleContainer.className = 'example-topics mt-2';
  exampleContainer.innerHTML = '<span class="example-label">Try:</span> ';
  
  exampleTopics.forEach((topic, index) => {
    const topicBtn = document.createElement('button');
    topicBtn.type = 'button';
    topicBtn.className = 'btn btn-sm btn-example';
    ...
    exampleContainer.appendChild(topicBtn);
    
    // Add spacing between buttons
    if (index < exampleTopics.length - 1) {
      exampleContainer.appendChild(document.createTextNode(' '));
    }
  });
  
  // Add example topics under the topic input field
  const topicFormGroup = document.getElementById('topic').closest('.form-group');
  if (topicFormGroup) {
    topicFormGroup.appendChild(exampleContainer);
  }
}
